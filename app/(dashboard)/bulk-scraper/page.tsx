    ...
    const downloadNovelInFrontend = async (novelInfo: any) => {
        try {
            ...
            for (const chapterSourceId of uniqueChapterIds) {
                ...
                let chapData: any = null;
                let chapterLoaded = false;

                // Retry ngắn cho từng chương để giảm thiếu chương do lỗi mạng tạm thời
                ...
                if (chapterLoaded) {
                    ...
                    failedChapterIds.push(chapterSourceId);
                }
            }
            ...
            if (failedChapterIds.length > 0 || chapCount !== uniqueChapterIds.length) {
                setProgressData(prev => ({ ...prev, [id]: { ...prev[id], downloaded: chapCount, total: uniqueChapterIds.length, status: "error" } }));
                toast.error(`Thiếu chương (${chapCount}/${uniqueChapterIds.length}), giữ lại trong Thư viện: ${novelObj.title}`);
                return;
            }

            setProgressData(prev => ({ ...prev, [id]: { ...prev[id], downloaded: chapCount, total: uniqueChapterIds.length, status: "done" } }));
