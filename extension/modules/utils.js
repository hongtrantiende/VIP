...
export function waitForTabLoad(tabId, targetUrl, ms = 30000) {
  return new Promise(async (resolve) => {
    ...
    // Check if the tab already matches target immediately to prevent race conditions
    try {
    ...
    async function fn(id, info) {
      if (id === tabId) {
        try {
          ...
          // Resolve immediately as soon as the tab navigates to the target URL.
          // This avoids getting blocked by slow loading background ads/assets on Chinese sites like 69shuba.
          if (isTarget) {
            chrome.tabs.onUpdated.removeListener(fn);
            clearTimeout(t);
            resolve();
