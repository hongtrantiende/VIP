...
export async function stvFetchChapter(payload, sendResponse) {
  try {
    ...
    if (!isFirstChapter) {
      ...
      // 1. Wait for URL to change from the old URL to ensure navigation has started
      if (oldUrl) {
      ...
      // 2. Wait for page load status to become complete
      ...
      for (let i = 0; i < maxContentWait / 1000; i++) {
        if (!stvScrapeActive) break;
        try {
          ...
          if (checkResp && checkResp.length > 200) {
            ...
            // Content must be different from previous chapter (only if prevContent is populated)
          ...
          // If port closed / page reloading, wait 2 seconds before checking again
          await delay(2000);
        }
      }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      try {
        ...
        // If connection fails (e.g. during page reload), wait longer for page to load
        await delay(2000);
        continue;
      }
      await delay(1000); // Normal poll delay if resolved but content not yet ready
    }
