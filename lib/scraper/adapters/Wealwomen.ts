...
export const WealwomenAdapter: SiteAdapter = {
  name: "Wealwomen / 番茄故事會",
  group: "cn",
  urlPattern: /wealwomen\.com/i,
  chapterWaitSelector: ".novelcontent",
  novelWaitSelector: "#pcChapterList li:not(.chapterListLoadingItem), .chapterList li:not(.chapterListLoadingItem)",
  ...
  async getNovelInfo(html, url) {
    ...
    if (ogTitle) {
      ...
      const titleEl = doc.querySelector(".bookName, .bookText h1, .bookTitle h2, h1");
      if (titleEl) {
    ...
    const container = doc.querySelector("#pcChapterList, .chapterList");
    ...
    links.forEach((link) => {
      ...
      // Smart title extraction: Use <em> or inner span if present
      const spanEl = link.querySelector(".catalogChapterNameFigma");
      const emEl = link.querySelector("em");
      let text = spanEl ? spanEl.textContent?.trim() || "" : (emEl ? emEl.textContent?.trim() || "" : "");
